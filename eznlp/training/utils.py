...
class LRLambda(object):
    ...
    @staticmethod
    def linear_decay_lr_with_warmup(num_warmup_steps: int,
                                    num_total_steps: int):
        assert num_warmup_steps >= 1
        ...
        def lr_lambda(step: int):
            if step < num_warmup_steps:
                ...
                return (num_total_steps - step) / (num_total_steps -
                                                   num_warmup_steps)
            else:
                return 0.0
    ...
    @staticmethod
    def exponential_decay_lr_with_warmup(num_warmup_steps: int,
                                         num_period_steps: int = None,
                                         gamma: float = 0.9):
...
def check_param_groups(model: torch.nn.Module,
                       param_groups: list,
                       verbose=True):
...
def count_params(model_or_params: Union[torch.nn.Module, torch.nn.Parameter,
