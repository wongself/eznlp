...
def parse_arguments(parser: argparse.ArgumentParser):
    ...
    group_data.add_argument('--dataset',
                            type=str,
                            default='conll2003',
                            help="dataset name")
    group_data.add_argument('--doc_level',
                            default=False,
                            action='store_true',
                            help="whether to load data at document level")
    group_data.add_argument('--corrupt_rate',
                            type=float,
                            default=0.0,
    ...
    group_data.add_argument(
        ...
        action='store_true',
        help=
        "whether to save predictions on the test split (typically in case without ground truth)"
    )
    group_data.add_argument(
        '--pipeline',
        default=False,
    ...
    group_decoder.add_argument('--ck_decoder',
                               type=str,
                               default='sequence_tagging',
                               help="chunk decoding method",
                               choices=[
                                   'sequence_tagging', 'span_classification',
                                   'boundary_selection'
                               ])
    # Loss
    group_decoder.add_argument('--fl_gamma',
                               type=float,
                               default=0.0,
                               help="Focal Loss gamma")
    group_decoder.add_argument('--sl_epsilon',
                               type=float,
                               default=0.0,
                               help="Label smoothing loss epsilon")

    # Sequence tagging
    group_decoder.add_argument('--scheme',
                               type=str,
                               default='BIOES',
                               help="sequence tagging scheme",
                               choices=['BIOES', 'BIO2'])
    group_decoder.add_argument('--no_crf',
                               dest='use_crf',
                               default=True,
                               action='store_false',
    ...
    # Span-based
    group_decoder.add_argument('--agg_mode',
                               type=str,
                               default='max_pooling',
                               help="aggregating mode")
    group_decoder.add_argument('--num_neg_chunks',
                               type=int,
                               default=100,
                               help="number of sampling negative chunks")
    group_decoder.add_argument('--max_span_size',
                               type=int,
                               default=10,
                               help="maximum span size")
    group_decoder.add_argument('--ck_size_emb_dim',
                               type=int,
                               default=25,
                               help="span size embedding dim")

    # Boundary selection
    group_decoder.add_argument('--no_biaffine',
                               dest='use_biaffine',
                               default=True,
                               action='store_false',
                               help="whether to use biaffine")
    group_decoder.add_argument('--affine_arch',
                               type=str,
                               default='FFN',
                               help="affine encoder architecture")
    group_decoder.add_argument('--neg_sampling_rate',
                               type=float,
                               default=1.0,
                               help="Negative sampling rate")
    group_decoder.add_argument('--hard_neg_sampling_rate',
                               type=float,
                               default=1.0,
                               help="Hard negative sampling rate")
    group_decoder.add_argument('--hard_neg_sampling_size',
                               type=int,
                               default=5,
                               help="Hard negative sampling window size")
    group_decoder.add_argument('--sb_epsilon',
                               type=float,
                               default=0.0,
                               help="Boundary smoothing loss epsilon")
    group_decoder.add_argument('--sb_size',
                               type=int,
                               default=1,
...
def collect_IE_assembly_config(args: argparse.Namespace):
    drop_rates = (0.0, 0.05,
                  args.drop_rate) if args.use_locked_drop else (args.drop_rate,
                                                                0.0, 0.0)

    ohots_config = {}
    if args.emb_dim > 0:
        vectors = load_vectors(args.language, args.emb_dim, unigram=True)
        ohots_config['text'] = OneHotConfig(field='text',
                                            vectors=vectors,
    ...
    if args.language.lower() == 'chinese' and args.use_bigram:
        ...
        ohots_config['bigram'] = OneHotConfig(field='bigram',
                                              vectors=vectors,
                                              emb_dim=50,
                                              freeze=args.emb_freeze)
    ...
    if args.language.lower() == 'chinese' and args.use_softword:
        mhots_config = ConfigDict(
            {'softword': MultiHotConfig(field='softword', use_emb=False)})
    else:
        mhots_config = None
    ...
    if args.language.lower() == 'english' and args.char_arch.lower() != 'none':
        ...
        nested_ohots_config = ConfigDict({
            'softlexicon':
            SoftLexiconConfig(vectors=vectors,
                              emb_dim=50,
                              freeze=args.emb_freeze)
        })
    else:
    ...
    if args.use_interm1:
        interm1_config = EncoderConfig(arch=args.enc_arch,
    ...
    if args.use_interm2:
        interm2_config = EncoderConfig(arch=args.enc_arch,
                                       hid_dim=args.hid_dim,
                                       num_layers=args.num_layers,
                                       in_drop_rates=drop_rates)
    else:
    ...
    if args.bert_arch.lower() != 'none':
        ...
        bert_like, tokenizer = load_pretrained(args.bert_arch,
                                               args,
                                               cased=True)
        bert_like_config = BertLikeConfig(
            ...
            freeze=False,
            use_truecase='cased'
            in os.path.basename(bert_like.name_or_path).split('-'))
    else:
...
def build_ER_config(args: argparse.Namespace):
    ...
    if args.ck_decoder == 'sequence_tagging':
        ...
        decoder_config = BoundarySelectionDecoderConfig(
            ...
            affine=EncoderConfig(arch=args.affine_arch,
                                 hid_dim=150,
                                 num_layers=1,
                                 in_drop_rates=(0.4, 0.0, 0.0),
                                 hid_drop_rate=0.2),
            fl_gamma=args.fl_gamma,
            sl_epsilon=args.sl_epsilon,
...
def process_IE_data(train_data, dev_data, test_data, args, config):
    if (config.bert_like is not None and
        ...
        train_data = segment_uniformly_for_bert_like(
            train_data,
            config.bert_like.tokenizer,
            update_raw_idx=True,
            verbose=args.log_terminal)
        dev_data = segment_uniformly_for_bert_like(dev_data,
                                                   config.bert_like.tokenizer,
                                                   update_raw_idx=True,
                                                   verbose=args.log_terminal)
        test_data = segment_uniformly_for_bert_like(test_data,
                                                    config.bert_like.tokenizer,
                                                    update_raw_idx=True,
                                                    verbose=args.log_terminal)
...
if __name__ == '__main__':
    ...
    device = torch.device('cuda',
                          args.gpu_id) if args.gpu_id >= 0 else auto_device()
    ...
    train_data, dev_data, test_data = process_IE_data(train_data, dev_data,
                                                      test_data, args, config)
    ...
    trainer.train_steps(train_loader=train_loader,
    ...
    evaluate_entity_recognition(trainer,
                                test_set,
                                batch_size=args.batch_size,
    ...
    if args.pipeline:
        ...
        for ex, chunks_pred in zip(train_data, train_set_chunks_pred):
            ex['chunks'] = ex['chunks'] + [
                ck for ck in chunks_pred if ck not in ex['chunks']
            ]
        ...
        torch.save((train_data, dev_data, test_data),
                   f"{save_path}/data-for-pipeline.pth")
